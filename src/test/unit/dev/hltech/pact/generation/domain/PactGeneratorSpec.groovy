...
import dev.hltech.pact.generation.domain.client.feign.multiple.FirstSampleSpecFeignClient
import dev.hltech.pact.generation.domain.client.feign.multiple.SecondSampleSpecFeignClient
...
class PactGeneratorSpec extends Specification {
    ...
    def "should merge 2 pacts with the same provider"() {
        ...
        and:
            feignClientsFinderMock.findFeignClients(_) >> Sets.newHashSet(FirstSampleSpecFeignClient.class, SecondSampleSpecFeignClient.class)
            pactFactoryMock.createFromFeignClient(FirstSampleSpecFeignClient.class, 'same-consumer', objectMapperMock) >> pact
            pactFactoryMock.createFromFeignClient(SecondSampleSpecFeignClient.class, 'same-consumer', objectMapperMock) >> anotherPact
